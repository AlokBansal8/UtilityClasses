...
@SuppressWarnings({ "WeakerAccess", "unused" }) public final class StringUtil {
  ...
  /**
   * Truncates the given string at given position, if position argument is greater than the length
   * of string argument whole string is returned.
   *
   * @param string text which needs to be truncated
   * @param position the position at which to truncate the string
   * @return truncated string
   */
  @CheckResult @NonNull public static String truncateAt(@NonNull final String string,
      @IntRange(from = 1) final int position) {
    return string.length() > position ? string.substring(0, position) : string;
  }
